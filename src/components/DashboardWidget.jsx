...
const DashboardWidget = React.forwardRef(({
    ...
    getUrl,
    refreshTrigger,
    ...gridProps  // All other props from react-grid-layout
}, ref) => {
    const {
        data,
        loading,
    ...
    return (
        <div
            ref={ref}
            {...gridProps}  // Spread grid layout props first
            id={`widget-${widget.i}`}
            className={`bg-white rounded-xl shadow-sm border border-slate-200 overflow-visible flex flex-col relative group h-full ${gridProps.className || ''}`}
        >
            {/* Widget Header */}
